...
describe('connection', () => {
  ...
  describe('configureFromEnv', () => {
    ...
    it('passes CDP_SECRET as header in direct mode', () => {
    ...
    it('respects custom DEV_BROWSER_PORT in managed mode', () => {
  ...
  describe('configure', () => {
    it('sets config and getConnectionMode reflects it', () => {
      configure({ mode: 'direct', cdpEndpoint: 'ws://x', taskId: 'task-1' });
      expect(getConnectionMode()).toBe('direct');
    });

    it('switches between modes', () => {
      configure({ mode: 'managed', devBrowserUrl: 'http://localhost:9224', taskId: 't' });
      expect(getConnectionMode()).toBe('managed');

      configure({ mode: 'direct', cdpEndpoint: 'ws://x', taskId: 't' });
      expect(getConnectionMode()).toBe('direct');
    });
  });
